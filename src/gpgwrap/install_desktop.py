...
def main() -> int:
    ...
    desktop_src = files("gpgwrap").joinpath("assets/gpgwrap.desktop")
    ...
    desktop_content = Path(desktop_src).read_text(encoding="utf-8")
    ...
    if gtk_update_icon_cache:
        subprocess.run(
            [gtk_update_icon_cache, "-f", "-t", str(Path.home() / ".local/share/icons/hicolor")],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
            check=False,
        )
